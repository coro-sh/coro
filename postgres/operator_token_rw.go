...
type OperatorTokenReadWriter struct {
	db   *sqlc.Queries
	txer *tx.PGXRepositoryTxer[tkn.OperatorTokenReadWriter]
}

func NewOperatorTokenReadWriter(db DB) *OperatorTokenReadWriter {
	return &OperatorTokenReadWriter{
		db: sqlc.New(db),
		txer: tx.NewPGXRepositoryTxer[tkn.OperatorTokenReadWriter](db, tx.PGXRepositoryTxerConfig[tkn.OperatorTokenReadWriter]{
			WithTxFunc: func(repo tkn.OperatorTokenReadWriter, txer *tx.PGXRepositoryTxer[tkn.OperatorTokenReadWriter], tx pgx.Tx) tkn.OperatorTokenReadWriter {
