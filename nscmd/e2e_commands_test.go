...
func TestStartConsumer(t *testing.T) {
	...
	user, err := entity.NewUser("consumer_user", h.ExistingAccount)
	require.NoError(t, err)
	...
	gotMsgsCh := make(chan *brokerv1.ReplyMessage, numMsgs)
	consumer, err := h.Commander.ConsumeStream(h.Operator.ID, user, streamCfg.Name, func(msg *brokerv1.ReplyMessage) {
		gotMsgsCh <- msg
	})
	require.NoError(t, err)
...
func TestConsumerHeartbeat(t *testing.T) {
	...
	gotRepliesCh := make(chan *brokerv1.ReplyMessage, 1)
	consumer, err := h.Commander.ConsumeStream(h.Operator.ID, user, streamCfg.Name, func(msg *brokerv1.ReplyMessage) {
		gotRepliesCh <- msg
	})
	require.NoError(t, err)
