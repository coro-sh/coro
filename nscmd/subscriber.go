...
type SubscriptionReplier func(ctx context.Context, msg *brokerv1.ReplyMessage) error

// SubscriberHandler processes received messages and returns reply(s).
type SubscriberHandler func(msg *brokerv1.PublishMessage, replier SubscriptionReplier) error
...
func (s *CommandSubscriber) Subscribe(ctx context.Context, handler SubscriberHandler) {
	go func() {
		...
		for {
			...
			pubMsg := &brokerv1.PublishMessage{}
			...
			err = func() error {
				...
				err = handler(pubMsg, func(ctx context.Context, replyMsg *brokerv1.ReplyMessage) error {
			...
			if err != nil {
				...
				errStr := err.Error()
				reply, err := proto.Marshal(&brokerv1.ReplyMessage{
					Id:    pubMsg.Id,
					Inbox: pubMsg.OperationReplyInbox,
					Error: &errStr,
				})
				if err != nil {
